...
function MyJourneyDrawer({ 
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-slate-950/80 backdrop-blur-md flex justify-end animate-fadeIn">
      ...
      <div className="w-full max-w-md bg-slate-900 border-l border-amber-500/30 h-full flex flex-col shadow-2xl animate-slideLeft">
        
        {/* Top Header */}
        <div className="p-5 bg-slate-950 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="w-9 h-9 rounded-xl bg-amber-500/20 border border-amber-500/40 text-amber-400 flex items-center justify-center font-bold text-base">
              🗺️
            </div>
            <div>
          ...
          {savedMonuments.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              {savedMonuments.map((monument, idx) => (
                <div
                  key={monument.id}
                  className="p-3.5 rounded-2xl glass-panel border border-slate-800 flex items-center justify-between gap-3 group hover:border-amber-500/40 transition-all"
                >
                  ...
                  <div className="flex items-center space-x-1 shrink-0">
                    <button
                      ...
                      className="p-2 rounded-lg bg-slate-800 hover:bg-amber-500 text-slate-300 hover:text-slate-950 text-xs font-bold"
                      title="View Details"
                    >
                      👁️
                    </button>
                    <button
                      ...
                      className="p-2 rounded-lg bg-slate-800 hover:bg-red-500 text-slate-400 hover:text-white text-xs font-bold"
                      title="Remove"
                    >
                      ✕
                    </button>
        ...
        {savedMonuments.length > 0 && (
          <div className="p-4 bg-slate-950 border-t border-slate-800 space-y-2">
            <button
              ...
              className="w-full py-3 rounded-xl bg-gold-gradient text-slate-950 font-bold text-xs shadow-lg shadow-amber-500/20 hover:brightness-110 flex items-center justify-center space-x-2"
            >
