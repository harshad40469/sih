...
function RecommendedSection({ 
  ...
  return (
    <section id="recommendations" className="py-20 relative bg-slate-950 border-t border-slate-800">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6 border-b border-slate-800 pb-8">
          ...
          <div className="flex items-center space-x-2 bg-slate-900/80 p-2.5 rounded-2xl border border-slate-800 text-xs text-slate-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {recommendedList.map((monument) => {
            const isSaved = journeyIds.includes(monument.id);

            return (
              <div
                key={monument.id}
                className="glass-panel glass-panel-hover rounded-3xl overflow-hidden border border-amber-500/20 flex flex-col justify-between group shadow-xl"
              >
                <div>
                  {/* Image Container */}
                  <div className="relative aspect-[16/10] overflow-hidden">
                    ...
                    {/* Match Score Badge */}
                    <div className="absolute top-3 left-3 glass-pill px-2.5 py-1 rounded-full text-xs font-bold text-amber-300 border border-amber-400/30 flex items-center space-x-1">
                      <span>⚡</span>
                      <span>{monument.matchScore}% Match</span>
                    </div>

                    {/* Category Pill */}
                    <div className="absolute top-3 right-3 glass-pill px-2.5 py-1 rounded-full text-[11px] font-semibold text-slate-200 border border-slate-700">
                      {monument.category}
                    </div>

                    {/* Duration Badge */}
                    <div className="absolute bottom-3 left-3 glass-pill px-2 py-0.5 rounded text-[11px] text-slate-300 flex items-center space-x-1">
                      <span>⏱️</span>
                ...
                <div className="p-5 pt-0 flex items-center space-x-2">
                  <button
                    ...
                    className="flex-1 py-2.5 rounded-xl bg-amber-500/20 hover:bg-amber-500 text-amber-300 hover:text-slate-950 font-bold text-xs border border-amber-500/40 transition-all flex items-center justify-center space-x-1"
                  >
